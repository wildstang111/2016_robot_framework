...
public class CrabDriveMode implements SwerveMode
{
   ...
   private static final double DEADBAND_SPEED = 0.02;
	...
	public SwerveBaseState calculateNewState(SwerveBaseState p_prevState, double... args) {
		...
		// Deadband check
		if (Math.abs(motorSpeed) < DEADBAND_SPEED) {
			if (Math.abs(rotation) > DEADBAND_SPEED) {
				int frontLeftAngle = (int) (Math.atan(length / width));
				int frontRightAngle = (int) (90 + Math.atan(width / length));
				newFrontRight.setRotationAngle(frontRightAngle);
				newFrontLeft.setRotationAngle(frontLeftAngle);
				newRearLeft.setRotationAngle(180 + frontRightAngle);
				newRearRight.setRotationAngle(180 + frontLeftAngle);

				newFrontLeft.setSpeed(rotation);
				newFrontRight.setSpeed(rotation);
				newRearLeft.setSpeed(rotation);
				newRearRight.setSpeed(rotation);
			} else {

				// If the calculated new speed is very low, leave the wheels at
				// their current angle, and set the
				// motor output to 0
			...
			// Crab drive is field oriented and does not allow the robot to
			// rotate
